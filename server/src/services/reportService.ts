...
export async function generateCSVReport(type: 'maintenance' | 'incidents' | 'assets' | 'executive'): Promise<string> {
  if (type === 'assets') {
    ...
    return json2csv(data, { fields: ['Metric', 'Value'] });
  }
}
...
export async function generatePDFReport(type: string, res: any): Promise<void> {
  ...
  doc.fillColor('#3b82f6').fontSize(22).text('ForgeMind AI', 50, 25, { bold: true });
  doc.fillColor('#8b96a8').fontSize(12).text('Industrial Knowledge Intelligence Platform', 50, 50);

  doc.moveDown(3);
  doc.fillColor('#111827').fontSize(18).text(`${type.toUpperCase()} REPORT`, 50, 100);
  doc.fontSize(10).fillColor('#6b7280').text(`Report Generated: ${new Date().toLocaleString()}`, 50, 122);
  ...
  if (type === 'assets') {
    ...
    doc.fontSize(11).fillColor('#111827').text('Asset ID', 50, y);
    doc.text('Zone', 140, y);
    ...
    assets.forEach((a) => {
      ...
      doc.fontSize(9).fillColor('#374151').text(a.id, 50, y);
    ...
    incidents.forEach((i) => {
      ...
      doc.fontSize(12).fillColor('#1d4ed8').text(`[${i.severity.toUpperCase()}] ${i.title}`, 50, y);
      doc.fontSize(9).fillColor('#6b7280').text(`Date: ${i.date} | Status: ${i.status}`, 50, y + 16);
      doc.fontSize(10).fillColor('#374151').text(`Description: ${i.description}`, 50, y + 30);
      if (i.rootCause) {
        doc.fontSize(9).fillColor('#dc2626').text(`Root Cause: ${i.rootCause}`, 50, y + 45);
        y += 15;
      }
      y += 55;
    });
  } else {
    doc.fontSize(12).fillColor('#374151').text('Executive Summary & Fleet Intelligence Overview', 50, 160);
    ...
    doc.moveDown(1);
    doc.fontSize(11).text(`• Total Fleet Registered Assets: ${total}`);
